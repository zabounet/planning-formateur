...
class RooterController extends Controller
{
    ...
    public function index()
    {
        if (isset($_SESSION['admin']) || isset($_SESSION['formateur'])) {
            ...
            $formateursList = $formateurModel->getFormateur();
            $GRNs = $formateurModel->getAll('GRN');
            $villes = $formation->getAll('Ville');

            $color = new CouleursModel;
            $color->setSessionCoulors();

            isset($_POST['rechercher']) ? $ogGrns = $_POST['grns'] : $ogGrns = $GRNs;
            if (isset($_POST['rechercher'])) $ogFormateurs = $_POST['formateurs'];
            else {
                for ($i = 0; $i < count($formateursList['Formateurs']); $i++) {
                    $idFormateurs = $formateursList['Formateurs'][$i]->id_formateur;
                    $ogFormateurs[] = $idFormateurs;
                }
            }

            isset($_POST['rechercher']) ? $ogNBFormateurs = $_POST['nbFormateurs'] : $ogNBFormateurs = count($ogFormateurs);
            isset($_POST['rechercher']) ? $ogVille = $_POST['lieu'] : $ogVille = $villes;
            ...
            $ogVille !== "default" ? $centre = $ogVille : $centre = "Aucun";
            ...
            if (empty($ogGrns)) {
                ...
                $ogGrns !== "default" ? $grn = $ogGrns : $grn = "Aucun";
            };

            // Si aucun formateur n'est selectionné, demande à l'utilisateur d'en selectionner au moins 1
            if (is_null($ogFormateurs)) {
                echo "Veuillez choisir au moins un formateur";
                exit;
            } else {
                $ogFormateurs === $ogNBFormateurs ? $formateursSelectionnes = "Aucun" : $formateursSelectionnes = $ogFormateurs;
            }
            ...
            $formateurs = $databaseFormateur->getDatesById(
                ['Formateur.id_formateur', 'nom_formateur', 'prenom_formateur', 'type_contrat_formateur', 'Formateur.date_debut_contrat', 'Formateur.date_fin_contrat', 'Formation.id_formation'],
            ...
            // Récupère l'ensemble des champs de la table formation où les numero_grn, id_formateur et id_ville sont respectivements égaux à $grn, $ogFormateurs et $centre. 
            $formations = $databaseFormation->getByIn(['*'], 'Formation', ['numero_grn', 'id_formateur', 'id_ville'], [$grn, $ogFormateurs, $centre]);
            ...
            if (Form::validate($_POST, ['rechercher'])) {
                ...
                // création d'un objet DateTime avec la date d'aujourd'hui
                $date_debut_tableau = new DateTime();
                // création d'un objet DateTime avec la date d'aujourd'hui + 2 mois
                $date_fin_tableau = clone $date_debut_tableau;
                $date_fin_tableau->add(new DateInterval('P2M'));
            }
            ...
            for ($x = 0; $x < $nbformation; $x++) {
                            ...
                            if ($weekendFormation === "6" || $weekendFormation === "7") {
                                ...
                                if ($interruption) {
                                    ...
                                    if ($pae) {
                                        ...
                                        if ($formations[$x]->id_type_formation == 4) {
                                            $html .= "<th style='background-color:" . $_SESSION['color']['couleur_pae'] . ";'></th> ";
                                        } else {
                ...
                for ($z = 0; $z < $countFormateurs; $z++) {
                    if (!($formateurs[$z]['date_fin_contrat'] < $date_debut_tableau && $formateurs[$z]['type_contrat_formateur'] != "CDI")) {
                        ...
                        for ($i = 0; $i < $nbJours; $i++) {
                            ...
                            $formateurAvoirAutre = false;
                            foreach ($formateur_autre[$formateurs[$z]['id_formateur']] as $autreFormateur) {
                                if ($periode >= $autreFormateur['debut_autre'] && $periode <= $autreFormateur['fin_autre']) {
                                    $formateurAvoirAutre = true;
                                    break;
                                }
                            }
                                    ...
                                    if ($formateurAvoirPeriode) {
                                        ...
                                        if (
                                            ...
                                            if ($jourLettre === "6" || $jourLettre === "7") {
                                                ...
                                                } else if ($formateurAvoirAutre) {
                                                    $html .= "<td style='background-color: " . $_SESSION['color']['couleur_autre'] . " ; box-shadow: inset 0 0 8px 2px #ffffffbf;'>" . $formateur_autre[$formateurs[$z]['id_formateur']][0]['lettre'] . "</td> ";
                                                } else if ($formateurAvoirTeletravail) {
                                                    $html .= "<td style='background-color: " . $_SESSION['color']['couleur_centre'] . " ; box-shadow: inset 0 0 8px 2px #ffffffbf;'> T </td> ";
                                                } else {
            ...
            if (Form::validate($_POST, ['valider'])) {
                ...
                switch ($type) {
                    case "Date_vacance":
                        ...
                        $notif_acceptee = $databaseFormateur->update($type, ['validation'], ["1"], ['id_formateur', 'date_debut_vacances', 'date_fin_vacances'], [$id_formateur, $date_array[0], $date_array[1]]);
                    ...
                    case "Date_teletravail":
                        $ttExists = $databaseFormateur->getByCustom(['validation'], 'Date_teletravail', ['id_formateur', 'validation'], ['=', '='], [$id_formateur, '1']);
                        if ($ttExists) {
                            $ttExists = $databaseFormateur->deleteCustom('Date_teletravail', ['validation', 'id_formateur'], ["=", "="], ['1', $id_formateur]);
                        }
                        $notif_refusee = $databaseFormateur->update($type, ['validation'], ["1"], ['id_formateur', 'jour_teletravail'], [$id_formateur, $date]);
            ...
            $notifications = $formateurModel->joinInformations(
            ...
            $this->render('main/index', compact('GRNs', 'formateursList', 'villes', 'html', 'data', 'notifs'), 'main');
        } else {
